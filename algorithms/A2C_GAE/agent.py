...
class Agent():
    ...
    def update(self):
        ...
        decay = torch.tensor(np.exp((np.array(range(len(self.memory)))*np.log(self.config.gamma * self.config.lamb)))).unsqueeze(1).to(device)
        advantage = (target - value) * decay
        advantage = torch.flip(advantage, [0])
        advantage = torch.cumsum(advantage, 0)
        advantage = torch.flip(advantage, [0])
        advantage = advantage / decay
        #print(target.shape, decay.shape, decay)
        loss1 = 0.5*torch.pow(advantage, 2)
        ...
        #print(probs)
        m = Categorical(probs)
        loss2 = m.log_prob(action.squeeze(1)).unsqueeze(1)*(advantage.detach())
