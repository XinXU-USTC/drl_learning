...
import random
from algorithms.ActorCritic.agent import Agent
...
class Runner():
    ...
    def train(self):
        ...
        agent.actor_net = nn.DataParallel(agent.actor_net, device_ids=range(config.train.device_ids))
        agent.critic_net = nn.DataParallel(agent.critic_net, device_ids=range(config.train.device_ids))
